...
NAME = "MURILLO SANTOS JAVIER"

def calculate_control(goal_x, goal_y, alpha, beta):
    v, w = 0, 0
    robot_x, robot_y, robot_a = get_robot_pose(listener)
    e_theta = math.atan2(goal_y - robot_y, goal_x - robot_x) - robot_a
    e_theta = (e_theta + math.pi) % (2 * math.pi) - math.pi
    v = v_max * math.exp(- e_theta ** 2 / alpha)
    w = w_max * (2 / (1 + math.exp(-e_theta / beta)) - 1)
    return [v, w]
