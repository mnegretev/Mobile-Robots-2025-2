...
class NeuralNetwork(object):
    ...
    def backpropagate(self, x, t):
        ...
        nabla_b[-1] = delta
        nabla_w[-1] = numpy.dot(delta.reshape(-1, 1), y[-2].reshape(1, -1))
        
        for l in range(2, len(self.biases) + 1):
            delta = numpy.dot(self.weights[-l+1].T, delta) * y[-l] * (1 - y[-l])
            nabla_b[-l] = delta
            nabla_w[-l] = numpy.dot(delta.reshape(-1, 1), y[-l-1].reshape(1, -1))        
...
def main():
    ...
    print("\nPress key to tst network or ESC to exit...")
    ...
    for tasa_aprendizaje in learning_rates:
        for epocas in epochs_list:
            for tamano_lote in batch_sizes:
                identificador += 1
                inicio_entrenamiento = time.time()
                nn.train_by_SGD(training_dataset, epocas, tamano_lote, tasa_aprendizaje)
                tiempo_entrenamiento = time.time() - inicio_entrenamiento

                exitos = 0
                for i in range(pruebas):
                    imagen, etiqueta = testing_dataset[numpy.random.randint(0, 4999)]
                    salida = nn.forward(imagen).transpose()

                    prediccion_correcta = etiqueta.transpose()[0][numpy.argmax(salida)] == 1
                    print("Éxito:", prediccion_correcta)

                    if prediccion_correcta:
                        exitos += 1

                tasa_exito = exitos / pruebas

                resultados.append({
                    "ID del Experimento": identificador,
                    "Tasa de Aprendizaje (η)": tasa_aprendizaje,
                    "Épocas": epocas,
                    "Tamaño de Lote": tamano_lote,
