...
def main():
    ...
    learning_rates = [0.1, 0.5, 1.0, 3.0, 5.0, 10.0]
    epochs_list    = [1, 3, 10, 20, 50, 70]
    batch_sizes    = [1, 5, 10, 30, 50, 70]
    total_tests    = 100

    with open("resultados_experimentos.csv", mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["test_number","learning_rate", "epochs", "batch_size", "train_time_sec", "correct_predictions", "total_tests", "accuracy_percent"])

        test_number = 1  # Contador para el número de prueba
        for lr in learning_rates:
            for ep in epochs_list:
                for bs in batch_sizes:
                    ...
                    print(f"\nTest #{test_number}: Training with lr={lr}, epochs={ep}, batch_size={bs}")
                    ...
                    print(f"Test #{test_number}: Accuracy: {accuracy:.2f}% | Time: {train_time:.2f} s")
                    writer.writerow([test_number, lr, ep, bs, round(train_time, 2), correct, total_tests, round(accuracy, 2)])
                    test_number += 1  # Incrementar el número de prueba
